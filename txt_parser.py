...
# class DJILocation(GEModel):
#     UpdateTime = GETimeField("更新时间")
#     Longitude = GECharField("经度")
#     Latitude = GECharField("纬度")
#     Distance = GECharField("飞行距离")
#     Height = GECharField("飞行高度(dm)")
            ...
            try:
                ...
                if recordType == RecordType.CUSTOM.value:
                    ...
                    if last_gps is not None:
                        # loc = DJILocation()
                        # loc.UpdateTime = update_time
                        # loc.Longitude = last_gps[0]
                        # loc.Latitude = last_gps[1]
                        # loc.Height = last_gps[2]
    ...
    def ParseDetail(self):
        ...
        try:
            ...
            if version < 0x06:
                ...
                eater.skip(7)
                # activeTimestamp: 8 bytes little-endian, in Unix time format:
...
# class DJIUserInfo(GEModel):
#     NickName = GECharField("昵称")
#     PhoneArea = GECharField("手机区号")
#     Phone = GECharField("手机号码")
#     UserID = GECharField("ID")
#     Email = GECharField("邮箱")

# class TXTDetailAttribute(GEModel):
#     Name = GECharField("名称")
#     Value = GECharField("值")
...
# def GEMain(frm):
#     root = frm.create_root_node("txt日志")
    
#     key_account_nickname = "djitest"
#     key_account_id = "djiuser-oruxe8p9vnkr"
#     key_account_email = "dc11c5153d344e1a8e9da3b7765b8aa6@sudo8diov.dji.com"
#     key_account_phone = "18817707586"
#     key_account_phone_area = "86"
#     user = root.create_node("用户信息")
#     test1 = user.create_node(key_account_nickname)
#     test1.add_data_item(DJIUserInfo(key_account_nickname,key_account_phone_area,
#                             key_account_phone,key_account_id,key_account_email))
    
#     for f in File.get_data(File.Ext=="txt"):
#         if not f["Name"].startswith("DJIFlightRecord"):
#             continue
#         print(f["Name"])
#         txt_node = root.create_node(f["Name"])
        
#         full_path = os.path.join(frm.get_mount_point(), f["FullPath"])
#         txt = TXTFile(full_path)
#         base_info = txt_node.create_node("基本信息")
#         detail = txt.ParseDetail()
#         timestamp = datetime.fromtimestamp(detail["timestamp"]/1000)
#         detail["timestamp"] = timestamp.strftime("%Y-%m-%d %H:%M:%S")
#         detail["photoNum"] = str(int(detail["photoNum"]))
#         detail["city"] = "上海市"
#         for key,value in detail.items():
#             cn_name = DetailNameMap.get(key, "")
#             if cn_name != "":
#                 base_info.add_data_item(TXTDetailAttribute(cn_name, value))
#         flight_info = txt_node.create_node("飞行日志")
#         for record in txt.ParseRecord():
#             flight_info.add_data_item(record)
#     root.save()
...
def ParseTxts():
    ...
    for fl in fls:
        ...
        print('需要解析的txt文件路径： ',full_path)
        # full_path = r"E:\Code\pycode\KMJ\DJIFlightRecord_2020-07-10_[20-31-19].txt"
        txt = TXTFile(full_path)
        # base_info = txt_node.create_node("基本信息")
        print("基本信息-----------------")
        detail = txt.ParseDetail()
