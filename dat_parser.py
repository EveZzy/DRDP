...
# Record_Status_OK = 1
# Record_Status_ROLLOVER = 2
# Record_Status_OUTOFORDER = 3
...
class DATFile:
    def __init__(self, data):
        self.data = data
        #print(acconfig.get(self.data[0], ("Unkown",4500000.0)))
        #print("FWData:", self.data[21:33].strip())
    ...
    def parse_gps_records(self):
        ...
        def get_payload(record):
            ...
            payload = bytes(map(lambda x:x^(record.ticket_no%256),payload))
            return payload
...
#     DateTime = GETimeField("时间")
#     Longtitude = GECharField("经度")
#     Latitude = GECharField("纬度")
    
# def GEMain(frm):
#     # root = frm.create_root_node("DJI飞行日志")
#     # mount_point = frm.get_mount_point()
#     # for f in File.get_data(File.Ext=="DAT", File.Type==model.FILE_TYPE_REG):
#     #     print(f["FullPath"])
#         # dat_path = os.path.join(mount_point, f["FullPath"])
#         dat_path = "20-07-10-08-31-09_FLY256.DAT"
#         print(dat_path)
#         # file_record = root.create_node(f["Name"])
...
def ParseDats():
    root_path = r"./dats" # 需要解析的dat文件放到dats目录下，可实现批量解析
    fls = os.listdir(root_path) # 遍历dats目录，获取其目录下的所有文件，返回的是文件名列表list
    for fl in fls:
